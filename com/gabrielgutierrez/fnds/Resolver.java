...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    public Void visitReturnStmt(Stmt.Return stmt) {
        if (currentFunction == FunctionType.NONE) {
            Foundations.error(stmt.keyword, "Cannot return from top-level code.");
        }

        if (stmt.value != null) {
            if (currentFunction == FunctionType.INITIALIZER) {
                Foundations.error(stmt.keyword,
    ...
    public Void visitAssignExpr(Expr.Assign expr) {
        if (preventAssignment)
            Foundations.error(expr.equals, "Assignment is not allowed within if, loop or ternary condition.");
    ...
    public Void visitSuperExpr(Expr.Super expr) {
        if (currentClass == ClassType.NONE) {
            Foundations.error(expr.keyword,
